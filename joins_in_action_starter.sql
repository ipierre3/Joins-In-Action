...
SELECT lead_studio, COUNT(lead_studio) AS most_favored FROM movies INNER JOIN users on movie_id = favorite_movie_id
GROUP BY lead_studio ORDER BY Most_Favored DESC LIMIT 1;

-- <<<<<<<<<<<<<<<<<<<<<< PROBLEM 5 >>>>>>>>>>>>>>>>>>>>>>>
-- Get the average Rotten Tomatoes score of all movies that are favorited by a user.
-- EXPECTED RESULT: 52.21

SELECT AVG(rotten_tomatoes) AS Avg_rotten_tomatoes_score FROM movies INNER JOIN users on movie_id = favorite_movie_id;
...
SELECT film, COUNT(film) AS duplicates FROM movies GROUP BY film HAVING COUNT(film) > 1;